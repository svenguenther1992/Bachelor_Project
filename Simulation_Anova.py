...
for a in distribution:
  for b in range(len(std_dev)):
    for c in sample_sizes:
      ...
      if c != 100:
        simulation_dict_unequal["ANOVA"].append(
            simulate_anova(data, distribution = a, repetition = 100,
                repetition_cv = 200, folds_cv = 5, sample_size = c, seed = 1,
                detailed = False)["ANOVA"])

        simulation_dict_unequal["Welch"].append(
            simulate_anova(data, distribution = a, repetition = 100,
                repetition_cv = 200, folds_cv = 5, sample_size = c, seed = 1,
                detailed = False)["Welch"])

        simulation_dict_unequal["Cross-Validation"].append(
            simulate_anova(data, distribution = a, repetition = 100,
                repetition_cv = 200, folds_cv = 5, sample_size = c, seed = 1,
                detailed = False)["Cross-Validation"])
...
for a in distribution:
  for b in range(len(std_dev)):
    for c in sample_sizes:
      ...
      if c != 100:
        simulation_dict_equal["ANOVA"].append(
            simulate_anova(data, distribution = a, repetition = 100,
                repetition_cv = 200, folds_cv = 5, sample_size = c, seed = 1,
                detailed = False)["ANOVA"])

        simulation_dict_equal["Welch"].append(
            simulate_anova(data, distribution = a, repetition = 100,
                repetition_cv = 200, folds_cv = 5, sample_size = c, seed = 1,
                detailed = False)["Welch"])

        simulation_dict_equal["Cross-Validation"].append(
            simulate_anova(data, distribution = a, repetition = 100,
                repetition_cv = 200, folds_cv = 5, sample_size = c, seed = 1,
                detailed = False)["Cross-Validation"])
